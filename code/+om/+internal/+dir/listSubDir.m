...
%   [absPath, dirName] = listSubDir(rootPath, expression, ignoreList, nRecurse)
    ...
    if isa(rootPath, 'cell') || isa(rootPath, 'string') && strlength(rootPath) > 1
        for i = 1:numel(rootPath)
            [absPathIter, dirNameIter] = om.dir.listSubDir(rootPath{i}, expression, ignoreList, nRecurse);
        ...
        if nRecurse > 0 && sum(keep) > 0 % Todo:
            [absPathRec, dirNameRec] = om.dir.listSubDir(absPath, expression, ignoreList, nRecurse-1);
