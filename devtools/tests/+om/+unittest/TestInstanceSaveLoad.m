classdef TestInstanceSaveLoad < matlab.unittest.TestCase
    % TestInstanceSaveLoad Unit test for save and load operations on
    % OpenMINDS types
    
    %Todo: 
    %   [ ] Move to openMINDS_MATLAB
    %   [ ] Add tests for opening editor per instance


    % Automatically generate a test case for each schema type
    properties (TestParameter)
        SchemaType = cellstr( enumeration('om.enum.Types') ); 
    end
    ...
    methods (Test)
        ...
        function testCreateSchema(testCase, SchemaType)
            ...
            schemaType = om.enum.Types(SchemaType);
